...
import json
from py.user_emails_dinosaurs import get_random_email_extension

first_names = {'m':[], 'f':[]}  # Make any name other.

with open('C:\\Users\\John\\Documents\\2016\\SQL Training 2016 Unicom\\Datasets\\Names\\names_by_sex.csv') as csvfile:

    reader = csv.DictReader(csvfile) #DictReader uses first column as the key
    for row in reader:
        m_or_f = row['sex']
        first_name = row['first_name']
        # print(m_or_f)
        # print(first_name)
        name_list = first_names.get(m_or_f)
        # print(name_list)
        name_list.append(first_name)
        # firstnames.append(row)
        # print(row['sex'], row['first_name'])
        '''my_reader = csv.reader(csvfile, delimiter=',') #  , quotechar='|'
        for row in my_reader:
        print (row)  #', '.join(row)'''

last_names = []
with open('C:\\Users\\John\\Documents\\2016\\SQL Training 2016 Unicom\\Datasets\\Names\\last_names.txt') as csvfile:
    reader = csv.DictReader(csvfile) #DictReader uses first column as the key
    for row in reader:
        ...
        for row in my_reader:
        print (row)  #', '.join(row)'''
# print(firstnames.get('m'))
# print(firstnames.get('f'))
